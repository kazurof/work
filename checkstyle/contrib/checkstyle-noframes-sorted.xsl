...
<xsl:template match="checkstyle">
      ...
      <tr>
        ...
        <th>Score</th>
          ...
          <xsl:for-each select="file[@name and generate-id(.) = generate-id(key('files', @name))]">
                ...
                <xsl:variable name="sumOfScore" select="sum(../file[@name=$currentName]/error/@score)"/>
                <tr>
          <xsl:call-template name="alternated-row"/>
					<td><a href="#f-{@name}"><xsl:value-of select="@name"/></a></td>
					<td><xsl:value-of select="$errorCount"/></td>
					<td><xsl:value-of select="$sumOfScore"/></td>
				</tr>
			</xsl:for-each>
		</table>
	</xsl:template>
    ...
    <a name="f-{@name}"></a>
    <h3>File <xsl:value-of select="@name"/>  (Score : <xsl:value-of select="sum(error/@score)"/>)</h3>
